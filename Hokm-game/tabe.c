...
{/*for finding the greatest element of an array*/
    int situation, i;
    for (i = 0; i < arrSize; i++)
    {
        situation = 1;
        ...
        if (situation)
        {
            *iGreatest = i;/*saving the index of the greatest element*/
            return arr[i];
        }
    }
}
...
{/*for finding the smallest element of an array*/
    ...
    for (i = 0; i < arrSize; i++)
    {
        ...
        if (situation == 1)
        {
            *iSmallest = i;/*saving the index of the smallest element*/
            return arr[i];
        }
    }
}
char Bakhshname(char *pointer)/*choosing the bakhshname by p1*/
{
    char bakhshname;
    printf("\nenter the bakhshname : ");
    scanf("%c", &bakhshname);
...
{/*for finding a symbol of a number*/
...
void player4(int cards[4][11], char bakhshname2, int index, char *symbolp4, int *Cardp4, char firstSymbol, int *flagp4)
{
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        {/*searching for the cards matching with the first symbol*/
            if (cards[3][i] < 44)/*checking if this card have been played before or not*/
            {
                cardp4[i] = (cards[3][i] % 11) + 1;
                *Cardp4 = findGreatest(cardp4, 11, &iGreatest4);/*find the greatest one*/
    ...
    for (int i = 0; i < 11; i++)
    {
        if (cardp4[i] != 0)/*if p4 didnt have any card matching with the first symbol*/
        {
            flag3 = 1;
            flag4 = 1;
            break;
    ...
    for (int i = 0; i < 11; i++)
    {
        if (flag3 == 0)
        {
            ...
            {/*searching for the cards matching with the bakhshname*/
                if (cards[3][i] < 44)/*checking if this card have been played before or not*/
                {
                    cardp4[i] = (cards[3][i] % 11) + 1;
                    *Cardp4 = findGreatest(cardp4, 11, &iGreatest4);/*find the greatest one*/
    ...
    for (int i = 0; i < 11; i++)
    {
        if (cardp4[i] != 0)/*if p4 didnt have any card matching with the bakhshname*/
        {
            flag4 = 1;
            break;
        }
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        {
            if ((cards[3][i] < symbolReverse(bakhshname2)) || (cards[3][i] >= symbolReverse(bakhshname2) + 11))
            {/*searching for the cards matching with the non_bakhshname*/
                if (cards[3][i] < 44)
                {/*checking if this card have been played before or not*/
                    symbolp4array[i] = (cards[3][i] / 11);
                    cardp4[i] = (cards[3][i] % 11) + 1;
                    *Cardp4 = findSmallest(cardp4, 11, &iSmallest4);/*find the smallest one*/
                    *symbolp4 = numToSymbol(symbolp4array[iSmallest4] * 11);/*find the symbol of the chosen card*/
                }
            }
        }
        else
...
{/*when p2 is starting the round*/
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        {/*searching for the cards with non_bakhshname symbol*/
            ...
            {/*checking if this card have been played before or not*/
                ...
                cardp2[i] = (cards[1][i] % 11) + 1;
                *Cardp2 = findGreatest(cardp2, 11, &iGreatest2start);/*find the greatest one*/
    ...
    {
        if (cardp2[i] != 0)
        {/*if p4 didnt have any card matching with the non_bakhshname symbol*/
            flag4 = 1;
            break;
        }
    }
    for (int i = 0; i < 11; i++)
    {
        ...
        {
            if ((cards[1][i] >= symbolReverse(bakhshname2)) && (cards[1][i] < symbolReverse(bakhshname2) + 11))
            {/*searching the cards with bakhshname symbol*/
                if (cards[1][i] < 44)
                {/*checking if this card have been played before or not*/
                    cardp2[i] = (cards[1][i] % 11) + 1;
                    *Cardp2 = findGreatest(cardp2, 11, &iGreatest2start);/*finding the greatest one*/
                    *symbolp2 = bakhshname2;
                }
            }
        }
    ...
    for (int i = 0; i < 11; i++)
    {
        if (cards[1][i] == ((cardp2[iGreatest2start] - 1) + (symbolReverse(*symbolp2))))
...
{/*as the same as the p2*/
    ...
    for (int i = 0; i < 11; i++)
    {
        if (cards[2][i] == ((cardp3[iGreatest3start] - 1) + (symbolReverse(*symbolp3))))
            cards[2][i] = 44;
    }
...
{/*as the same as the p2*/
    ...
    for (int i = 0; i < 11; i++)
    {
        if (cards[3][i] == ((cardp4[iGreatest4start] - 1) + (symbolReverse(*symbolp4))))
            cards[3][i] = 44;
    }
...
{/*printing the playground*/
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        printf("%c  | ", Symbol(cards, 0, i));/*printing the first 5 symbol of p1*/
    }
    printf("\n        ");
    for (int i = 0; i < 54; i++)
    {
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        {
            printf("%d | ", subMultiple);/*printing the first 5 number of p1*/
        }
        else
        {
            printf("%d  | ", subMultiple);/*printing the first 5 number of p1*/
...
void zaminBazi2(int cards[4][11], int winnerTeam1, int winnerTeam2, int Cardp2, int Cardp3, int Cardp4, char symbolp2,
                char symbolp3, char symbolp4, int index, int flagp1, int flagp4, int flagp3, int flagp2, int Index[11])
{/*printing the playground during the rounds*/
    ...
    printf("    team 1 : %d\n", winnerTeam1);/*printing the point of the first team*/
    printf("      |");
    for (int j = 0; j < 25; j++)
    {
        printf(" ");
    ...
    printf("    team 2 : %d\n", winnerTeam2);/*printing the point of the second team*/
    for (int i = 0; i < 33; i++)
    {
        printf(" ");
    }
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        for (int j = 0; j < 11; j++)
        {
            if (i == Index[j])
            {
                printf("-  | ");/*printing - instead of cards symbols which have chosen before */
                symbolflag = 0;
                break;
            }
        }
        if (symbolflag)
        {
            printf("%c  | ", Symbol(cards, 0, i));/*printing the symbol of the rest cards*/
        }
    }
    printf("\n        ");
    for (int i = 0; i < 54; i++)
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        if (numberflag)
        {
            ...
            if (subMultiple / 10 > 0)/*our number have 2 digits so we print with one less space*/
            {
                printf("%d | ", subMultiple);/*printing the number of the rest cards*/
            }
            else
            {
                printf("%d  | ", subMultiple);/*printing the number of the rest cards*/
...
void checkingP1(int cards[4][11], int firstSymbol, int *index, int Index[11], int *flagp1, int flagcheck3)
{
    ...
    for (int j = 0; j < 100; j++)
    {
        for (int i = 0; i < 11; i++)
        {
            while (index1 == Index[i])
            {/*which means p1 has played this card before*/
        ...
        while (index1 < 0 || index1 > 11)
        { /*p1 should choose an index between 0 an 11*/
            printf("\nwrong index");
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        {/*we call the players in order : p1, p2, p3, p4*/
            ...
            zaminBazi2(cards, winnerTeam1, winnerTeam2, Cardp2, Cardp3, Cardp4, symbolp2, symbolp3, symbolp4, index, flagp1,
                       flagp4, flagp3, flagp2, Index);
            scanf("%c", &space);
            player2(cards, bakhshname2, index, &symbolp2, &Cardp2, firstSymbol, &flagp2);
            zaminBazi2(cards, winnerTeam1, winnerTeam2, Cardp2, Cardp3, Cardp4, symbolp2, symbolp3, symbolp4, index, flagp1,
                       flagp4, flagp3, flagp2, Index);
            player3(cards, bakhshname2, index, &symbolp3, &Cardp3, firstSymbol, &flagp3);
            zaminBazi2(cards, winnerTeam1, winnerTeam2, Cardp2, Cardp3, Cardp4, symbolp2, symbolp3, symbolp4, index, flagp1,
                       flagp4, flagp3, flagp2, Index);
            player4(cards, bakhshname2, index, &symbolp4, &Cardp4, firstSymbol, &flagp4);
            zaminBazi2(cards, winnerTeam1, winnerTeam2, Cardp2, Cardp3, Cardp4, symbolp2, symbolp3, symbolp4, index, flagp1,
                       flagp4, flagp3, flagp2, Index);
        }
        if (winnerPerson == 2)
        ...
        zaminBazi2(cards, winnerTeam1, winnerTeam2, Cardp2, Cardp3, Cardp4, symbolp2, symbolp3, symbolp4, index, flagp1,
                   flagp4, flagp3, flagp2, Index);
        if (winnerTeam1 == 6 || winnerTeam2 == 6)/*the round is over*/
        {
            break; 
        }
